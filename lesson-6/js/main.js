...
let btnStart = document.getElementById('start'), // Начать расчет   
    ...
    chosePercent = document.querySelector('.choose-percent'), // Накопления Процентd
    ...
    window.addEventListener('load', function() {
        ...
        optionalExpBtn.setAttribute('disabled', '');
        countBudgetBtn.setAttribute('disabled', '');
        choseIncome.setAttribute('readonly', '');
        checkSavings.setAttribute('disabled', '');
        choseSum.setAttribute('readonly', '');
        chosePercent.setAttribute('readonly', '');
    });
...
let money, time;
 ...
 btnStart.addEventListener("click", function() {
   ...
   dayValue.value = new Date(Date.parse(time)).getDate();
	...
	optionalExpBtn.removeAttribute('disabled', '');
	countBudgetBtn.removeAttribute('disabled', '');
	choseIncome.removeAttribute('readonly', '');
	checkSavings.removeAttribute('disabled', '');
	choseSum.removeAttribute('readonly', '');
	chosePercent.removeAttribute('readonly', '');

  	for (let i = 0; i < exspensesItem.length; i++) {

		exspensesItem[i].addEventListener('input', function () {
...
countBudgetBtn.addEventListener("click", function() {
   ...
   if(appData.budget != undefined) {
     ...
     let sumItem = +exspensesItem[1].value + +exspensesItem[3].value;

      appData.moneyPerDay = ((appData.budget - sumItem) / 30).toFixed();
